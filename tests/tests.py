...
class ResoTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.admin_manager.add_account(cls.test_hash)

    def test_manager_start(self) -> None:
        """Test availability of pinned message."""
        cookies = self.admin_manager.get_cookies(self.test_hash)
        self.assertEqual(len(cookies), 2)

    def test_accounts_managing(self) -> None:
        """Test add and remove account methods."""
        self.admin_manager.remove_account(self.test_hash)
        # cookies = self.manager.get_cookies(self.test_hash)
        # self.assertEqual(len(cookies), 2)
        self.admin_manager.add_account(self.test_hash)
        cookies = self.admin_manager.get_cookies(self.test_hash)
        self.assertEqual(len(cookies), 2)

    def test_launch(self) -> None:
        """Test browser application launch."""
        self.assertTrue(self.admin_manager.get_cookies(self.test_hash))
    ...
    def tearDownClass(cls) -> None:
        ...
        cls.admin_manager.remove_account(cls.test_hash)
